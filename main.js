...
let light=100,count=0,status='playing',cooldown=0,pulseAge=99,time=0,toastTimer=0,yaw=-.14,pitch=.37,distance=30,drag=false,lastX=0,lastY=0,audioOn=false,audioCtx=null;
const keys=new Set(),stick={x:0,y:0};let last=performance.now();const camTarget=new THREE.Vector3(0,2.15,2);
function tone(freq,duration=.25,type='sine',volume=.05){if(!audioOn)return;audioCtx??=new AudioContext();audioCtx.resume();const o=audioCtx.createOscillator(),g=audioCtx.createGain();o.type=type;o.frequency.setValueAtTime(freq,audioCtx.currentTime);o.frequency.exponentialRampToValueAtTime(freq*.65,audioCtx.currentTime+duration);g.gain.setValueAtTime(volume,audioCtx.currentTime);g.gain.exponentialRampToValueAtTime(.001,audioCtx.currentTime+duration);o.connect(g);g.connect(audioCtx.destination);o.start();o.stop(audioCtx.currentTime+duration)}
...
$('pause').onclick=()=>modal(status==='paused'?'playing':'paused');$('resume').onclick=()=>modal('playing');$('restart').onclick=restart;$('pulse').onclick=pulse;$('audio').onclick=()=>{audioOn=!audioOn;$('audio').textContent='Âm thanh: '+(audioOn?'bật':'tắt');$('audio').setAttribute('aria-pressed',String(audioOn));tone(420)};
...
restart();modal('paused');$('modalTitle').textContent='Đánh thức cổng cổ';$('resume').textContent='Bắt đầu hành trình';requestAnimationFrame(frame);
